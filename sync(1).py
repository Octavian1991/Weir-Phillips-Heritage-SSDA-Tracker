...
def detail(html, row):
    ...
    # Real Portal descriptions overwhelmingly say "apartments", not "units" or
    # "dwellings" (e.g. "552 apartments within 56 and 60 storey buildings"),
    # and "storey" (singular) is at least as common as "storeys". The
    # original patterns matched neither on real, live examples - broadened
    # here, but this remains a best-effort parse of free text written
    # differently by every applicant/planner, not a structured field:
    # multi-tower descriptions ("56 and 60 storey buildings") only yield
    # whichever single number happens to sit next to the keyword, and if an
    # "affordable" subset count is mentioned before the project's total
    # count, the subset gets picked up instead of the total. Flag these
    # figures to users as unverified, not authoritative.
    m = re.search(r"\b(\d[\d,]*)\s+(?:new\s+|residential\s+)?(?:units|dwellings|apartments|residences|homes)\b", desc, re.I)
    if m:
        dwell = float(m.group(1).replace(",", ""))
    m = re.search(r"\b(\d+(?:\.\d+)?)\s*(?:storeys?|stories|story)\b", desc, re.I)
    if m:
    ...
    m = re.search(r"(?:affordable housing|affordable dwellings|affordable homes|affordable apartments)[^\d]{0,40}(\d[\d,]*)", desc, re.I)
    if m:
...
def save_project(c, d):
    ...
    # Select by explicit column name, not `SELECT *`. Columns added later via
    # `ALTER TABLE ADD COLUMN` (like gfa, coordinate_source) are appended to
    # the END of the table's physical column order, regardless of where they
    # sit in the `columns` list above. `SELECT *` would zip this list against
    # that physical order and silently shift every value from that point on
    # into the wrong field - which is exactly how a timestamp previously
    # ended up in the `applicant` column. Naming columns explicitly makes the
    # row order match `columns` unconditionally.
    old = c.execute(
        f"SELECT {','.join(columns)} FROM projects WHERE project_number=?",
        (d["project_number"],),
    ).fetchone()
